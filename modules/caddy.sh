...
caddy_install() {
    ...
    if ! curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg; then
        log_error "GPG 密钥下载或导入失败，请检查网络连接"
        return 1
    fi

    # 添加官方 APT 源
    log_step "添加 Caddy 官方 APT 源..."
    if ! curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | tee /etc/apt/sources.list.d/caddy-stable.list > /dev/null; then
        log_error "APT 源添加失败，请检查网络连接"
        return 1
    fi
