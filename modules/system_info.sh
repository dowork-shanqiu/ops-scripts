...
SYSTEM_INFO_MODULE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
source "${SYSTEM_INFO_MODULE_DIR}/common.sh"
...
sys_resources() {
    ...
    ps aux --sort=-%cpu 2>/dev/null | sed -n '1,11p' || true
    print_thin_separator
    ...
    print_thin_separator
    ps aux --sort=-%mem 2>/dev/null | sed -n '1,11p' || true
    print_thin_separator
...
sys_network() {
    ...
    # 默认路由
    log_step "默认路由:"
    ip route 2>/dev/null | sed -n '1,5p' || true

    echo ""

    # DNS 配置
    log_step "DNS 配置:"
    awk '!/^[[:space:]]*#/ && NF' /etc/resolv.conf 2>/dev/null || true
    ...
    log_step "公网 IP:"
    local public_ip
    public_ip=$(curl -s --connect-timeout 5 https://ipinfo.io/ip 2>/dev/null || true)
    if [ -n "$public_ip" ]; then
        echo "  ${public_ip}"
    else
        echo "  无法获取公网 IP"
...
sys_disk() {
    ...
    log_step "磁盘使用情况:"
    print_thin_separator
    df -hT 2>/dev/null | awk '$2 != "tmpfs" && $2 != "devtmpfs"' || true
    print_thin_separator
    ...
    log_step "Inode 使用情况:"
    print_thin_separator
    df -i 2>/dev/null | awk '$1 !~ /^(tmpfs|devtmpfs)$/' || true
    print_thin_separator
    ...
    print_thin_separator
    du -sh /* 2>/dev/null | sort -rh | sed -n '1,10p' || true
    print_thin_separator
